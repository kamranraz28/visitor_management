    ...
    @if(session('success_message'))
        ...
        <div class="row justify-content-end mt-3">
            <a style="width: 150px; height: 40px; display: flex; justify-content: center; align-items: center; margin: 0 20px 10px 0;"
               href="{{route('new_department_add')}}" class="btn btn-secondary btn-sm">Add new Department</a>
        </div>
                                ...
                                <div class="table-responsive">
                                    ...
                                    @foreach ($department as $dept)
                                        <tr style="text-align:center">
                                            ...
                                            <td>{{ $dept->name }}</td>
                                            <td>
                                                <button class="btn btn-secondary editButton" data-id="{{ $dept->id }}">Edit</button>
                                                <a href="{{ route('department_delete', ['id' => $dept->id]) }}" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this department?')">Delete</a>
                                            </td>
  ...
  document.addEventListener('DOMContentLoaded', function() {
   document.querySelectorAll('.editButton').forEach(function(button) {
      button.addEventListener('click', function() {
         ...
         var department_id = clickedElement.dataset.id;
         Swal.fire({
